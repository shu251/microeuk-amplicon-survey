...
### Adonis
Use ```adonis()``` to examine categorical significance. Use ```dist_euc_noboca``` or ```dist_euc_nobocaBSW```.
```{r}
set.seed(1212)

adonis(dist_euc_noboca ~ sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$SITE, permutations = 999) # Categorical with vent site
```
...
```{r}
adonis(dist_euc_nobocaBSW ~ sample_data(subset_samples(physeq_insitu, (VENT != "Boca" & SAMPLETYPE == "Vent")))$SITE, permutations = 999) # Categorical with vent site
```
...
```{r}
# vent_microeuk_rda_sampletype <- dbrda(dist_euc_noboca ~ temp + ph + persea + mg + h2 + h2s + ch4 + prok + Condition(sampletype), dist = 'euc', na.action = na.exclude)

# anova(vent_microeuk_rda_sampletype, by = 'terms')
```

Try with condition set to site.
```{r}
vent_microeuk_rda_site <- dbrda(dist_euc_noboca ~ temp + ph + persea + mg + h2 + h2s + ch4 + prok + Condition(site), dist = 'euc', na.action = na.exclude)

anova(vent_microeuk_rda_site, by = 'terms')
```
...
## Stats for Axial
Subset to only Axial samples (remove Boca outlier) - check significance of year. Subset to vents only - check significant of year again.
```{r}
AXIAL <- subset_samples(physeq_insitu, (SITE == "Axial" & VENT != "Boca"))
...
```{r}
# dbrda_subset(subset_samples(AXIAL, SAMPLETYPE == "Vent"))
```
...
When we isolate only resident taxa (ASV detected only at vent sites), significance geochem parameters include temperature, percent seawater, H2, and CH4.
...
```
Stramenopiles, MAST mostly: Temperature, H2, and CH4 were found to be significant drivers of stramenopile diversity and distribution.
```{r}
dbrda_subset(subset_taxa(noboca, Phylum == "Sagenista"))
dbrda_subset(subset_taxa(ventonly_noboca, Phylum == "Sagenista"))
```
Radiolaria: no geochemical parameters drove radiolaria population distribution.
```{r}
dbrda_subset(subset_taxa(noboca, Phylum == "Radiolaria"))
dbrda_subset(subset_taxa(ventonly_noboca, Phylum == "Radiolaria"))
```
