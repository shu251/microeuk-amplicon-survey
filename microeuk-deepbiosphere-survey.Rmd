...
Establish overall community composition.
...
ggplot(clr_trans_insitu_asv, aes(x = SAMPLETYPE, y = CLR, fill = SUPERGROUP, group = sampletype_tax)) +
  # geom_jitter(width = 0.3, size = 0.3) +
  geom_hline(yintercept = 0) +
  # geom_boxplot(aes(group = sampletype_tax), width = 0.5) +
  geom_violin(aes(group = sampletype_tax)) +
  coord_flip() +
  facet_grid(SUPERGROUP ~ SITE, space = "free") +
...
clr_trans_insitu_asv %>% 
  filter(SUPERGROUP != "Protalveolata") %>% 
  group_by(SAMPLETYPE, SUPERGROUP, SITE_ORDER) %>% 
    summarize(COUNT_SUM = sum(PRESENCE))  %>% 
ggplot(aes(x = SAMPLETYPE, y = SITE_ORDER, fill = SUPERGROUP)) +
...
```{r}
# head(df_wide_num)
df_wide_vent <- df_wide_num %>% 
  rownames_to_column(var = "SAMPLE") %>% 
  filter(grepl("Vent", SAMPLE)) %>% 
  column_to_rownames(var = "SAMPLE")
...
clr_to_plot_maintax <- function(df, slice, tax_slice){
  ...
  rownames_to_column(var = "VENTNAME") %>%
    ...
    ggplot(aes(x = CLASS2, y = VENTNAME, fill = CLR)) +
      ...
      theme(legend.position = "right", 
            ...
            strip.background = element_blank(), 
            strip.text.y = element_text(hjust = 0, vjust = 0.5, angle = 0, size = 10, color = "black"),
            strip.text.x = element_text(hjust = 1, vjust = 1, angle = 90, size = 10, color = "black"),
            legend.title = element_blank(),
            strip.placement = "outside") + 
      labs(x = "", y = "") +
    scale_fill_steps2(
  ...
  aesthetics = "fill") +
    coord_flip() +
    facet_grid(PHYLUM ~ ., space = "free", scales = "free", switch = "both")
}
```
...
maintext_supergroup <- c("Alveolata-Ciliophora", "Alveolata-Dinoflagellata", "Rhizaria-Cercozoa", "Rhizaria-Radiolaria")
clr_to_plot_maintax(insitu_asv_wClass, cosmo, maintext_supergroup)
```
...
# Isolate vent-only (putative endemic)
...
endemic_processed <- endemic %>% 
  ...
  mutate(CLASS = case_when(
    ...
    is.na(Class) ~ Phylum,
    grepl("MAST-", Class) ~ "MAST",
    grepl("CONTH_", Class) ~ "CONThreeP",
    grepl("RAD-", Class) ~ "RAD-A, -B, -C",
    grepl("MOCH-", Class) ~ "MOCH-2, -5",
    TRUE ~ Class
  )) %>% 
  filter(SUPERGROUP != "Archaeplastida") %>%
  filter(!(grepl("Other", SUPERGROUP))) %>% 
  filter(!(grepl("Other", PHYLUM))) %>% 
  # Average across replicates
    ...
    # mutate(PREVALENCE = rowSums(select_if(., is.numeric) > 0)) %>% head
    #   filter(PREVALENCE >= 1) %>% select(-PREVALENCE) %>% 
    column_to_rownames(var = "CLASS")
# dim(endemic_processed)
# head(endemic_processed)
# View(endemic_processed %>% 
#         rownames_to_column(var = "CLASS") %>% 
#         separate(CLASS, c("SUPERGROUP", "PHYLUM", "CLASS"), sep = "_") %>% 
#         select(SUPERGROUP, PHYLUM, CLASS) %>% distinct())
...
maintext_supergroup <- c("Alveolata-Ciliophora", "Alveolata-Dinoflagellata", "Rhizaria-Cercozoa", "Rhizaria-Radiolaria")
# svg("tileplot-endemic-bysample-FULL.svg", h = 10, w = 20)
# svg("tileplot-endemic-bysample-SUBSET.svg", h = 10, w = 20)
data.frame(compositions::clr(endemic_processed)) %>% 
    ...
    filter(Supergroup %in% maintext_supergroup) %>% 
...
Repeat above plot for supplemental
...
# colnames(endemic)
# svg("env-heatmap-legend.svg", w = 15, h = 4)
plot_grid(
 endemic_env("TEMP") + 
...
plot_sampletype <- function(df){
  ...
  ggplot(aes(x = SAMPLETYPE_ORDER, y = Shannon, group = SAMPLETYPE_ORDER)) +
    ...
    geom_jitter(color = "#525252", width = 0.4, size = 2, aes(fill = SITE_ORDER, shape = SAMPLETYPE_ORDER)) +
...
#           plot_sampletype(spp_alpha_TYPE_SITE),
#           ncol = 2)
```
...
#         panel.grid.major = element_line(),
#         axis.line = element_line()) +
#   facet_grid(SITE + SAMPLETYPE ~ ., scales = "free", space = "free") +
#     labs(x = "", y = "Shannon")

```

```{r, fig.height=4, fig.width=5}
# plot_grid(spp_alpha_TYPE_SITE %>% 
#   select(SITE, SAMPLETYPE, starts_with("Shannon"), starts_with("Simpson")) %>% 
#   distinct() %>% 
#   ggplot(aes(x = SAMPLETYPE, y = Shannon, fill = SITE, shape = SAMPLETYPE)) +
#   geom_errorbar(aes(ymin = `Shannon-lower`, ymax = `Shannon-upper`), color = "#525252", width = 0.2) +
#   geom_point(color = "#525252", size = 2, aes(fill = SITE, shape = SAMPLETYPE)) +
#   scale_shape_manual(values = sampletype_symbol) +
#   scale_fill_manual(values = site_color) +
...
#   select(SITE, SAMPLETYPE, starts_with("Shannon"), starts_with("Simpson")) %>% 
#   distinct() %>% 
#   ggplot(aes(x = SAMPLETYPE, y = Simpson, fill = SITE, shape = SAMPLETYPE)) +
#   geom_errorbar(aes(ymin = `Simpson-lower`, ymax = `Simpson-upper`), color = "#525252", width = 0.2) +
#   geom_point(color = "#525252", size = 2, aes(fill = SITE, shape = SAMPLETYPE)) +
#   scale_shape_manual(values = sampletype_symbol) +
#   scale_fill_manual(values = site_color) +
...
```{r}
dbrda_subset(subset_samples(physeq_insitu, (VENT != "Boca")))
```
#### Vent only
Repeat without boca, and look at only vent sites
```{r}
...
to_supergroup_CLASS %>% 
  ...
  ggplot(aes(area = value, fill = SUPERGROUP, subgroup = SUPERGROUP)) +
    geom_treemap(color = "white") +
    geom_treemap_subgroup_border(colour = "white", size = 2) +
    # geom_bar(stat = "identity", position = "fill", color = "black", width = 0.9) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        ...
        panel.border = element_blank()) +
  scale_y_continuous(expand = c(0,0)) +
  scale_fill_manual(values = c("#fa9fb5", "#c51b8a", "#67000d", "#ef3b2c", "#ffffcc", "#feb24c", "#c7e9b4", "#1d91c0", "#253494", "#9e9ac8", "#238b45", "#54278f", "#bdbdbd", "#252525", "#f0f0f0")) +
  labs(x = "", y = "")
...
# View(supergroup_distri_bubble)
```


```{r, fig.height=4, fig.width=7}
ggplot(supergroup_distri_bubble, aes(y = res_asv_perc, x = cosmo_asv_perc, size = SUPERGROUP_ASV_TOTAL, color = SUPERGROUP)) +
  # geom_abline(slope = 1, intercept = 0, alpha = 0.5) +
  geom_point(shape = 21, stat = "identity", stroke = 3, fill = NA) +
  scale_size_continuous(range = c(2,22)) +
  scale_y_continuous(limits = c(0,100)) +
  scale_x_continuous(limits = c(0,100)) +
  theme_linedraw() +
  scale_color_manual(values = c("#fa9fb5", "#c51b8a", "#67000d", "#ef3b2c", "#ffffcc", "#feb24c", "#c7e9b4", "#1d91c0", "#253494", "#9e9ac8", "#238b45", "#54278f", "#bdbdbd", "#252525", "#f0f0f0")) +
  labs(x = "Proportion of cosmopolitan ASVs", y = "Proportion of resident ASVs") +
  theme(legend.title = element_blank())
```
```{r}
ggplot(supergroup_distri_bubble, aes(x = ASV_TOTAL_Cosmopolitan, y = ASV_TOTAL_Resident, size = SUPERGROUP_SEQ_TOTAL, fill = SUPERGROUP)) +
  # geom_abline(slope = 1, intercept = 0, alpha = 0.5) +
...
```{r}
noNAs <- filter(insitu_asv_wClass, (Supergroup != "Bacteria_X" | !is.na(Supergroup) | Supergroup != "Terrabacteria" | Supergroup != "Archaea_X" | Supergroup != "Opisthokonta"))
...
cos <- filter(insitu_asv_wClass, CLASS != "Vent only")
...
cosmo <- prune_taxa(list_of_cos, physeq_insitu)
...
Manually cycle through these and set df to each. Need to look at individual outputs.
```{r}
# df <- physeq_insitu
# df <- noNAs_phy
# df <- res_vent
...
Test with data set, vents only.
```{r}
# allsamples <- dbrda_subset(subset_samples(df, (VENT != "Boca"))); allsamples
# ventonly <- dbrda_subset(subset_samples(df, (SAMPLETYPE == "Vent" & VENT != "Boca")));ventonly
# mcr <- dbrda_subset(subset_samples(df, (SITE == "VonDamm" | SITE == "Piccard" | SAMPLETYPE == "Vent")));mcr
# 
# NEpac <- dbrda_subset(subset_samples(df, (SITE == "GordaRidge" | SITE == "Axial" & VENT != "Boca" | SAMPLETYPE == "Vent"))) ; NEpac
# vd <- dbrda_subset(subset_samples(df, (SITE == "VonDamm" | SAMPLETYPE == "Vent"))); vd
# picc <- dbrda_subset(subset_samples(df, (SITE == "Piccard" | SAMPLETYPE == "Vent"))); picc
# axial <- dbrda_subset(subset_samples(df, (VENT != "Boca" & SITE == "Axial" | SAMPLETYPE == "Vent"))); axial
# gr <- dbrda_subset(subset_samples(df, (SITE == "GordaRidge" | SAMPLETYPE == "Vent"))); gr
```

Repeat this process, but now include all samples at individual vent sites. 
```{r}
# df <- physeq_insitu
# df <- noNAs_phy
# df <- res_vent
...
```{r}
# mcr <- dbrda_subset(subset_samples(df, (SITE == "VonDamm" | SITE == "Piccard")));mcr
# NEpac <- dbrda_subset(subset_samples(df, (SITE == "GordaRidge" | SITE == "Axial" & VENT != "Boca"))) ; NEpac
# vd <- dbrda_subset(subset_samples(df, (SITE == "VonDamm"))); vd
# picc <- dbrda_subset(subset_samples(df, (SITE == "Piccard"))); picc
# axial <- dbrda_subset(subset_samples(df, (VENT != "Boca" & SITE == "Axial"))); axial
# gr <- dbrda_subset(subset_samples(df, (SITE == "GordaRidge"))); gr
```
# Table generation of ASV categories

```{r}
# site_class_select <- c("All sites", "Axial & Gorda Ridge", "Piccard & Von Damm")
# 
# asv_list <- insitu_asv_wClass %>% 
#   select(FeatureID, Taxon, CLASS, SITE_CLASS, Domain:Species) %>% 
#   filter(SITE_CLASS %in% site_class_select) %>% 
...
# dim(asv_list)
# names(asv_list)
# write_delim(asv_list, file = "output-tables/shared-asv-list.txt", delim = "\t")
```

```{r, fig.height=10, fig.width=7}
# asv_list %>% 
#   group_by(Phylum, CLASS, SITE_CLASS) %>% 
#   summarise(COUNT = n()) %>% 
#   ggplot(aes(x = CLASS, y = COUNT, fill = Phylum)) +
#   geom_bar(stat = "identity", position = "fill", color = "black", width = 0.4) +
#   theme_linedraw() +
#   theme(axis.text.x = element_text(angle = 90)) +
#   facet_grid(SITE_CLASS ~ .)
```

```{r}
# tmp <- filter(asv_list, grepl("MAST", Taxon))
# View(tmp)
```

```{r}
# asv_list %>% 
#   filter(!(SITE_CLASS == "All sites")) %>% 
#   group_by(Phylum, CLASS, SITE_CLASS) %>% 
#   summarise(COUNT = n()) %>% 
#   # filter(SITE_CLASS == "All sites") %>% 
#   ggplot(aes(x = SITE_CLASS, y = COUNT, fill = Phylum)) +
#   geom_bar(stat = "identity", position = "fill", color = "black", width = 0.4) +
#   theme_linedraw() +
#   theme(axis.text.x = element_text(angle = 90)) +
#   facet_wrap(CLASS ~ .)
