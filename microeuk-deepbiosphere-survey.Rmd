...
```{r Barplot all, tidy=FALSE, fig.height=6, fig.width=13}
all <- c("Axial", "VonDamm", "Piccard", "GordaRidge")
...
insitu_asv_wClass %>% 
  ...
  ggplot(aes(x = SAMPLE)) +
    geom_bar(color = "black", width = 0.5, aes(fill = SUPERGROUP_ORDER)) +
...
supergroup_distri_bubble %>% 
  ...
  geom_point(shape = 21, stat = "identity") +
...
dist_compiled_output_vents %>% 
  ...
  theme(strip.background = element_blank(),
        strip.text = element_text(color = "black")) +
  labs(y = "Distance metric", title = "Distance-Decay: Vent samples only")
```
...
dist_compiled_output_cosmo %>% 
  ...
  theme(strip.background = element_blank(),
        strip.text = element_text(color = "black")) +
  labs(y = "Distance metric", title = "Distance-Decay: Cosmopolitan ASVs")
```
  ...
  rownames_to_column(var = "VENTNAME") %>%
    ...
    ggplot(aes(x = CLASS2, y = VENTNAME, fill = CLR)) +
      ...
      theme(legend.position = "right", 
            ...
            strip.text.x = element_text(hjust = 1, vjust = 1, angle = 0, size = 10, color = "black"),
            legend.title = element_blank(),
            strip.placement = "outside") + 
      labs(x = "", y = "") +
    scale_fill_steps2(
...
```{r make upsetR plot for MCR, fig.width=14, fig.height=8}
