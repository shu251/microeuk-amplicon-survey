...
# Distance metric by distance km
...
axial_vents <- insitu_asv_wClass %>% 
  left_join(metadata %>% select(SAMPLE, COORDINATES)) %>% 
...
### Plot resident comparison
```{r, fig.height=7, fig.width=8}
# head(joined_dist)
# sites_order
joined_dist$X_SITE_ORDER <- factor(joined_dist$X_SITE, levels = sites_order)
joined_dist$Y_SITE_ORDER <- factor(joined_dist$Y_SITE, levels = rev(sites_order))
joined_dist %>% 
...
ggplot(aes(x = X, y = Y, fill = Jaccard_CLR_metric)) +
  ...
  scale_fill_gradientn(colors = c("#fff7f3", "#fde0dd", "#fcc5c0", "#fa9fb5", "#f768a1", "#dd3497", "#ae017e", "#7a0177", "#49006a")) +
  # coord_fixed(ratio = 1) +
  facet_grid(Y_SITE_ORDER ~ X_SITE_ORDER, scales = "free", space = "free", switch = "both")

```

### Plot cosmopolitan comparison
```{r, fig.height=7, fig.width=8}
# head(joined_dist)
# sites_order
joined_dist$X_SITE_ORDER <- factor(joined_dist$X_SITE, levels = (sites_order))
...
## Whole community comparison
```{r}
all_classes <- as.character(unique(insitu_asv_wClass$CLASS))

jacc_allsamples <- isolate_calc_jaccard(insitu_asv_wClass, all_classes)
```

```{r}
dist_jacc_clr_all <- as.data.frame(as.matrix(vegdist(compositions::clr(jacc_allsamples), method = "euclidean"))) %>%
    rownames_to_column(var = "start") %>% 
    pivot_longer(cols = row.names(cosmopolitan), names_to = "end") %>% 
    select(start, end, Jaccard_CLR_metric = value) %>% 
    mutate(VARS = purrr::map2_chr(start, end, ~toString(sort(c(.x, .y))))) %>%
...
# head(dist_jacc_clr_all)
```
```{r, fig.height=10, fig.width=12}

dist_jacc_clr_all$X_SITE_ORDER <- factor(dist_jacc_clr_all$X_SITE, levels = (sites_order))
dist_jacc_clr_all$Y_SITE_ORDER <- factor(dist_jacc_clr_all$Y_SITE, levels = rev(sites_order))
...
# Numerics - geochem
# use dist_euc_noboca
# and: sample_data(subset_samples(physeq_insitu, (VENT != "Boca"))

temp <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$TEMP
ph <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$pH
persea <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$PercSeawater
mg <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$Mg
h2 <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$H2
h2s <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$H2S
ch4 <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$CH4
prok <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$ProkConc

# categorical (logic)
sampletype <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$SAMPLETYPE
site <- sample_data(subset_samples(physeq_insitu, (VENT != "Boca")))$SITE
```
...
```{r}
# ?dbrda()
vent_microeuk_rda <- dbrda(dist_euc_noboca ~ temp + ph + persea + mg + h2 + h2s + ch4 + prok, dist = 'euc', na.action = na.exclude)
...
With various outliers depending on the question posed, below function takes subsetted data and runs ```dbrda()``` analysis.
...
```
Compare with original (redundant to above) with only removal of Boca outlier.
```{r}
...
# Run with Boca outlier removed.
dbrda_subset(subset_samples(res_vent, (SAMPLETYPE == "Vent" & VENT != "Boca")))
```

### Isolate individual taxa

Run similar analysis, but isolate individual taxa. First isolate phyloseq objects to be without the Boca outlier and one with only vent sites.
```{r}
# head(tax_table(physeq_insitu))
ventonly_noboca <- subset_samples(physeq_insitu, (SAMPLETYPE == "Vent" & VENT != "Boca"))

noboca <- subset_samples(physeq_insitu, (VENT != "Boca"))
```
Ciliates: geochemistry and related parameters did not correspond to ciliate population.
```{r}
dbrda_subset(subset_taxa(noboca, (Phylum == "Ciliophora")))
dbrda_subset(subset_taxa(ventonly_noboca, (Phylum == "Ciliophora")))
```
