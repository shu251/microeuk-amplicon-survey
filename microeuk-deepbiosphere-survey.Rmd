...
# svg("../../../Manuscripts_presentations_reviews/deepsea-microeuk-ME/vectorgraphic-figures/fig5a-bubble.svg", h = 4, w = 4)
supergroup_distri_bubble %>% 
  ...
  geom_point(shape = 21, stat = "identity", color = "white") +
...
vent_dist_decay <- dist_compiled_output_vents %>% 
  ...
  theme(strip.background = element_blank(),
        strip.text = element_text(color = "black")) +
  labs(y = "Jaccard Dissimilarity", title = "Distance-Decay: Vent samples only")
```
...
cosmo_dist_decay <- dist_compiled_output_cosmo %>% 
  ...
  theme(strip.background = element_blank(),
        strip.text = element_text(color = "black")) +
  labs(y = "Jaccard Dissimilarity", title = "Distance-Decay: Cosmopolitan ASVs")
```

Figure S6
```{r, fig.height=8, fig.width=6}
vent_dist_decay + cosmo_dist_decay +patchwork::plot_layout(nrow = 2)
```
...
plot_sampletype <- function(df){
  ...
  ggplot(aes(x = SAMPLETYPE_ORDER, y = Shannon, group = SAMPLETYPE_ORDER)) +
    ...
    geom_jitter(color = "white", fill = "black", width = 0.4, size = 2, aes(shape = SITE_ORDER)) +
  ...
  df %>%
    ...
    geom_jitter(color = "white", fill = "black", width = 0.4, size = 2.5, aes(shape = SITE_ORDER)) +
    scale_shape_manual(values = site_symbol) +
  theme_linedraw() +
...
# svg("../../../Manuscripts_presentations_reviews/deepsea-microeuk-ME/vectorgraphic-figures/fig4b-shannon-wlegend.svg", h = 4, w = 5)
spp_alpha_label %>%
  ggplot(aes(x = SAMPLETYPE_ORDER, y = Shannon, group = SAMPLETYPE_ORDER)) +
    geom_violin(color = "#525252", fill = "#525252", alpha = 0.2, width = 0.6, draw_quantiles = c(0.25, 0.5, 0.75)) +
    geom_jitter(color = "white", fill = "black", width = 0.4, size = 3, aes(shape = SITE_ORDER)) +
...
Figure S6.
```{r, fig.height=7, fig.width=9}
# svg("../../../Manuscripts_presentations_reviews/deepsea-microeuk-ME/vectorgraphic-figures/figs6-distdecay-shannon-panel.svg", h = 7, w = 9)
(vent_dist_decay / cosmo_dist_decay) | shannon_divnet +patchwork::plot_layout() + plot_annotation(tag_levels = "a")
# dev.off()
```
...
dbrda_subset <- function(subset_df){
  ...
  # DBRDA
  rda_out <- dbrda(euc_out ~ temp + ph + persea + mg + h2 + h2s + ch4 + prok + depth, dist = 'euc', na.action = na.exclude)
